class Shift < ActiveRecord::Base
  ...
  def deliver_to!(places: [])
    ...
    stop_delivering_to = self.delivery_places.where.not(place_id: places)
    ...
    if self.halt? && places.count != self.delivery_places.count
  ...
  def update_delivery_times!(zero = current_delivery_place.current_index)
    places = self.delivery_places.order("current_index ASC").to_a
    count = places.count
  ...
  validates :state, uniqueness: { scope: [:courier], message: "You already have an outstanding shift. Call us for help #{COMPANY_PHONE}." }, if: :active?

  validate :cannot_end_with_pending_orders!, if: :ended?

  def cannot_end_with_pending_orders!
    errors.add(:state, "cannot be ended when there are still orders to deliver") if self.orders.active.count > 0
  end
